...
def create_recipe(conn, cursor):
    # Store all ingredients of the unique recipe
    recipe_ingredients = []
    name = str(input("\nEnter the name of the recipe: "))
    cooking_time = int(input("Cooking time in minutes: "))
    ingredient = input("Enter ingredients(separated by a comma): ")
    recipe_ingredients.append(ingredient)
    difficulty = calculate_difficulty(cooking_time, recipe_ingredients)

    # Store ingredients into a comma separated string so that the recipe_ingredients can be stored in database
...
def search_recipe(conn, cursor):
    ...
    # Get all the ingredients and separate them with comma and store them into the all_ingredients list
...
def update_recipe(conn, cursor):
    ...
    if column_for_update == "name":
        cursor.execute(
            "UPDATE Recipes SET name = %s WHERE id = %s", (updated_value, id_for_update)
        )
        ...
        cursor.execute(
            ...
            (updated_difficulty, id_for_update),
        )
